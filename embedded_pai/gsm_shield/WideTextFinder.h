...
private:
  ...
  int startMillis; // used for timeout measurement, base value
  boolean debug;
  const unsigned long BASE_VALUE=30000;// the maxiam response time is 300ms , referring to the manufactor's document
...
public:
  ...
  WideTextFinder(NewSoftSerial &stream, int timeout = 0);          // Ethernet constructor

  //~WideTextFinder();
  
  boolean read(char &recaivedByte);             // private function to read from the stream
