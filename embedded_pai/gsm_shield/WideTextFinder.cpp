...
// the constructors allow one and only one of the streams to be initialized
  ...
  WideTextFinder::WideTextFinder(NewSoftSerial &stream, int timeout) : nSerialStream(&stream) 
  { 
    this->timeout = timeout * 1000L;     
	debug=true;
  }

  //WideTextFinder:: ~WideTextFinder(){}
  ...
  boolean  WideTextFinder::findUntil(char *target, char *terminate)
  {
    ...
    unsigned char index = 0;  // maximum target string length is 255 bytes
